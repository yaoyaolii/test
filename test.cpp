...
static std::string prefix = "/Users/liyao/Project/6D-Pose/shape_based_matching-master/test/";
...
//    mode = "test";
    if(mode == "train"){
        ...
        // Mat img = cv::imread(prefix+"test1/train_1.png");
        ...
        Mat test_img = imread(prefix+"case0/4.png");
...
//    mode = "test";
    if(mode == "train"){
        Mat img = imread(prefix+"test1/train_1.png");
        assert(!img.empty() && "check your img path");
        ...
        detector.writeClasses(prefix+"test1/%s_templ.yaml");
        shapes.save_infos(infos_have_templ, prefix + "test1/test_info.yaml");
        std::cout << "train end" << std::endl << std::endl;
    }else if(mode=="test"){
        std::vector<std::string> ids;
        ids.push_back("test");
