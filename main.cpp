...
//  line2d
//
//  Created by liyao on 2020/3/13.
//  Copyright © 2020 liyao. All rights reserved.
//
...
static std::string prefix = "/Users/liyao/Project/6D-Pose/line2d/line2d/test/";
...
//    mode = "test";
    if(mode == "train"){
        ...
        Mat img = cv::imread(prefix+"scale2/test_1.jpg");
        ...
        // save templates
        detector.writeClasses(prefix+"scale2/%s_templ.yaml");

        // save infos,
        // in this simple case infos are not used
        shapes.save_infos(infos_have_templ, prefix + "scale2/circle_info.yaml");
        ...
        Mat test_img = imread(prefix+"case0/5.jpg");
        ...
        for(size_t i=0; i<top5; i++){
            ...
            for(int i=0; i<templ[0].features.size(); i++){
                ...
                cv::circle(img, {feat.x+match.x, feat.y+match.y}, 2, color, -1);
            }
        }
...
//    mode = "test";
    if(mode == "train"){
        Mat img = imread(prefix+"test3/train.jpg");
        assert(!img.empty() && "check your img path");
        //Mat mask = imread(prefix+"test3/mask.jpg");
        //assert(!mask.empty() && "check your img path");

        Rect roi(137, 114, 290, 290);
        img = img(roi).clone();
        //mask = mask(roi).clone();
        ...
        shapes.angle_step = 3;
        // scale
        shapes.scale_range = {0.7f, 1.3f};
        shapes.scale_step = 0.02f;
        shapes.produce_infos();
        std::vector<shape_based_matching::shapeInfo_producer::Info> infos_have_templ;
        ...
        detector.writeClasses(prefix+"test5/%s_templ.yaml");
        shapes.save_infos(infos_have_templ, prefix + "test5/test_info.yaml");
        std::cout << "train end" << std::endl << std::endl;
    }else if(mode=="test"){
        std::vector<std::string> ids;
        ids.push_back("test");
        detector.readClasses(ids, prefix+"test3/%s_templ_1.yaml");

        // angle & scale are saved here, fetched by match id
        auto infos = shape_based_matching::shapeInfo_producer::load_infos(prefix + "test3/test_info_1.yaml");

        Mat test_img = imread(prefix+"test3/test_5.jpg");
        assert(!test_img.empty() && "check your img path");
        Mat mask = imread(prefix+"test3/mask.jpg");
        assert(!mask.empty() && "check your img path");
        ...
        cv::Mat padded_mask = cv::Mat(mask.rows + 2*padding,
                                     mask.cols + 2*padding, mask.type(), cv::Scalar::all(0));
        mask.copyTo(padded_mask(Rect(padding, padding, test_img.cols, test_img.rows)));
        ...
        Mat mask_test = padded_mask(roi).clone();
        
        Mat output;
        Mat mask_out = Mat::zeros(img.size(), CV_8UC3);
        assert(img.isContinuous());
...
//        cvtColor(img, img, CV_BGR2GRAY);
        ...
        Timer timer;
        float threshold = 80;
        auto matches = detector.match(img, threshold, ids);
        timer.out();
        ...
        for(size_t i=0; i<top5; i++){
            ...
            float r_scaled = 290/2.0f*infos[match.template_id].scale;

            // scaling won't affect this, because it has been determined by warpAffine
            // cv::warpAffine(src, dst, rot_mat, src.size()); last param
            // float train_img_half_width = 180/2.0f + 100;
            float train_img_half_width = 290/2.0f;
            ...
            double scale = infos[match.template_id].scale;
            cv::Size src_sz = mask_test.size();
            cv::Size dst_sz(src_sz.width, src_sz.height);
            //int len = std::max(src_sz.width, src_sz.height);
            //平移
            cv::Mat t_mat =cv::Mat::zeros(2, 3, CV_32FC1);
            t_mat.at<float>(0, 0) = 1;
            t_mat.at<float>(0, 2) = match.x - 237; //水平平移量
            t_mat.at<float>(1, 1) = 1;
            t_mat.at<float>(1, 2) = match.y - 214; //竖直平移量
            cv::warpAffine(mask_test, dst, t_mat, dst_sz);
            
            //指定旋转中心
            //cv::Point2f center(src_sz.width / 2. + match.x - 137, src_sz.height / 2. + match.y - 114);
            cv::Point2f center(x,y);
            //获取旋转矩阵（2x3矩阵）
            cv::Mat rot_mat = cv::getRotationMatrix2D(center, angle, scale);
            //根据旋转矩阵进行仿射变换
            cv::warpAffine(dst, dst, rot_mat, dst_sz);
        ...
        Mat channel[3];
        split(mask_out, channel);
        channel[0] = Mat::zeros(mask_out.rows, mask_out.cols, CV_8UC1);
        channel[1] = Mat::zeros(mask_out.rows, mask_out.cols, CV_8UC1);
        merge(channel,3,mask_out);
        cv::addWeighted(img, 0.7f, mask_out, 0.3f, 0, output);
        imshow("img", output);
        waitKey(0);
